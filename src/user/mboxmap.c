...
int
xsend(int id, void *data, size_t len) {
    struct mboxbuf buf;
...
int
xrecv(int id, void *data, size_t size, size_t *lenp) {
    struct mboxbuf buf;
...
filter_main(int *data, int mbox_data, int mbox[]) {
    ...
    while (xrecv(mbox_data, data, size, &len) == 0) {
        ...
        for (i = 0; i < mod; i ++) {
            xsend(mbox[i], count + i, sizeof(int));
        }
    }
}
...
select_main(int mbox, int pid) {
    size_t len;
    int count = 0, ans;
    while (xrecv(mbox, &ans, sizeof(int), &len) == 0) {
